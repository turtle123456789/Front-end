...
export const getProductType = async (sort, page, limit,choice) => {
    if (sort) {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-all?limit=${limit}&page=${page}&sort=${choice}&sort=${sort}`)
        return res.data
    }
}
